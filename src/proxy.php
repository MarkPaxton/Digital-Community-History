...
if(empty($data)) {
  ...
  // post XML
  $posting = new HTTP_Client($host, $port, $data, 600, '128.243.21.248', 3128);
...
class HTTP_Client { 
  ...
  function HTTP_Client($host, $port, $data, $timeout = 30, $proxy_host=NULL, $proxy_port=NULL) { 
    $this->host = $host; 
    $this->port = $port;
    $this->data = $data;
  ...
  //Opens a connection 
  function connect() { 
    $this->socket = fsockopen($this->proxy_host, 
      $this->proxy_port, 
      $this->errno, 
      $this->errstr, 
      $this->timeout 
  ...
  function request($data) { 
    $this->buf = ""; 
    fwrite($this->socket, 
      "POST http://$this->host:$this->port$this->path HTTP/1.0\r\n". 
      "Host:$this->host\r\n". 
...
function send_request() {
  ...
  curl_setopt($ch, CURLOPT_PROXY, "http://wwwcache.cs.nott.ac.uk:3128");
